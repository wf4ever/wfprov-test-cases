# $Id: TestUtils.py 1367 2010-07-27 15:31:19Z graham $
#
...
def getTestSuite(testclass,testdict,select="unit"):
    ...
    select      is the test suite selector:
                ...
                "times"     same as "all": return suite of unit and component tests
                name        a single named test to be run
    """
    suite = unittest.TestSuite()
    # Named test only
    if select[0:3] not in ["uni","com","all","int","pen","tim"]:
...
def runTests(logname, getSuite, args):
    ...
    if len(args) > 1:
        sel = args[1]
    if sel[0:3] in ["uni","com","all","int","pen","tim"]:
